 ...
 */
export const handleBridge = async function (
  wallet,
  provider,
  auth,
  paymentToken,
    ...
    if (paymentToken != "ETH" && paymentToken != "MATIC" && paymentToken != "gasless") {
      ...
      if (iscontract) {
        ...
        return {
          success: false,
          error:
            "Its a known bug that first transaction of a fun wallet would fail if you are covering gas using ERC20 tokens. Please try to pay gas using gasless paymaster or ETH for this transaction and try token paymaster later.",
        };
      }
