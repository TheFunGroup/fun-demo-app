...
export default function WalletView(props) {
  ...
  useOnClickOutside(dropdownRef, (e) => {
    if (
      walletBtnRef?.current?.contains(e.target) ||
      e.target == walletBtnRef?.current
    )
      return;
    setDropdown(false);
  });
  ...
  return (
    <div>
      {addr && (
        <div
          className="w-[164px] flex items-center cursor-pointer"
          onClick={handleDropdown}
          ref={walletBtnRef}
        >
          <Image src="/profile.svg" width="28" height="28" alt="" />
          <div className="text-[#667085] text-sm mx-2 font-mono max-w-[104px]">{`${addr.substring(
            0,
            5
          )}...${addr.substring(addr.length - 4)}`}</div>
          <Image
            src="/chevron.svg"
            width="20"
            height="20"
      ...
      {dropdown && (
        <div
          className="dropdown w-[343px] absolute mt-2 -ml-[172px] pt-0 "
          ref={dropdownRef}
        >
          {!showSettings && (
            <div className="flex flex-col items-center w-full">
              <div className="flex items-center w-full justify-between py-[18px] px-6 border-b-[1px] border-[#00000014]">
                <div className="flex items-center text-sm">
                  <div className="text-black mr-2 whitespace-nowrap font-bold">
                    Fun Wallet
                  </div>
                  <div
                  ...
                  >
                    {showCopy && (
                      <div className="copy">
                        {copying ? "Copied!" : "Click to Copy"}
                      </div>
                    )}
                    <div className="font-mono text-[#667085] mr-1">{`${addr.substring(
                      0,
                      5
                    )}...${addr.substring(addr.length - 4)}`}</div>
                  </div>
              ...
              {tab == "balance" && (
                <div className="p-6 pt-0 w-full flex items-center flex-col max-h-[427px] overflow-y-scroll">
                  ...
                  <div className="flex items-end">
                    <div className="text-[32px] font-semibold mr-1">
                      {Number(balance).toFixed(6)}
                    </div>
                    <div className="text-[#667085] mb-2">
                      {networks[network]?.nativeCurrency.symbol}
                    </div>
                  </div>
                  <div className="text-[#667085] text-lg -mt-1">{`$${balanceUSD} USD`}</div>
                  <div
                    className="button-dark text-center py-3 px-4 w-full mt-6 font-medium"
                    onClick={handleFund}
                  >
                    Fund
                  </div>
                  <div className="self-start text-black text-lg font-[590] mt-6 mb-2">
                    Coins
                  </div>
              ...
              {tab == "nfts" && (
                <div className="w-full max-h-[427px] min-h-[427px] overflow-y-scroll p-6 pt-0 flex flex-col items-center">
                  <div className="w-full flex items-center justify-between my-6">
                    <div className="flex items-center">
                      <div className="text-xl font-semibold text-black mr-1">
                        NFTs
                      </div>
                      <div className="text-[#74777C] text-xl font-semibold">
                        {nfts.length}
                      </div>
                    </div>
                    <div
                    ...
                    >
                      ...
                      <div className="text-[#2D4EA2] text-sm font-semibold ml-1">
                        Mint an NFT
                      </div>
                    </div>
                  </div>
          ...
          {showSettings && (
            <div className="flex flex-col items-center w-full">
              <div className="flex items-center w-full justify-between py-[19px] px-6 border-b-[1px] border-[#00000014]">
                <div className="text-black mr-2 whitespace-nowrap font-bold text-sm">
                  Settings
                </div>
                <Image
                  src="/close.svg"
