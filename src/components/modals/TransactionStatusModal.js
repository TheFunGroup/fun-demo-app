...
const TransactionStatusRow = (props) => {
  let title = (
    <div className="flex justify-start items-center">
      ...
      <div className={"text-textBlue-200 text-base font-regular px-4"}>
        {props.title}
      </div>
    </div>
  );
  if (props.title === "Pending") {
    title = (
      <div className="flex justify-start items-center">
        ...
        <div
          className={
            "text-textBlue-200 text-base font-regular px-4 animate-pulse"
          }
        >
          {props.title}
        </div>
  ...
  if (props.title === "Completed" && props.status !== "completed") {
    title = (
      <div className="flex justify-start items-center opacity-70">
        ...
        <div className={"text-text-100 text-base font-regular px-4 "}>
          {props.title}
        </div>
      </div>
    );
    ...
    <div className="flex w-full justify-between items-center">
      ...
      <div className="flex ">
        {props.date && props.time ? (
          <>
            {" "}
            <div className="text-text-100 text-base">
              {props.date || "-- -- --"}
            </div>
            <div className="text-text-100 text-base px-2">{" · "}</div>
            <div className="text-text-200 text-base font-medium">
              {props.time || "-- --"}
            </div>
          </>
        ) : (
...
const TransactionStatusModal = (props) => {
  ...
  useEffect(() => {
    ...
    if (
      explorerURL?.slice(explorerURL.length - 11, explorerURL.length) ===
      "#internaltx"
    ) {
  ...
  return (
    <div className={`modal w-[690px] my-12 font-sfpro`}>
      <div className="flex justify-between items-start w-full">
        <div>
          <div className="text-[#101428] font-semibold text-xl px-6 pt-6">
            {title || "Staking"}
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-col items-start justify-evenly w-1/3 min-h-[148px]">
          ...
          {!(networkIn && networkOut) ? (
            <NetworkDisplay
              token={tokenIn || "ETH"}
              size="32"
              classes={" text-xl max-w-[142px] h-[48px]"}
            />
          ) : (
            <NetworkDisplay
              network={networkIn || "ethereum"}
              size="32"
              classes={" text-xl max-w-[142px] h-[48px]"}
            />
          )}
      ...
      <div className="w-full flex justify-between items-center mt-2 p-6 ">
        ...
        <Link
          href={
            explorerURL ||
            "https://goerli.etherscan.io/tx/0xf32673f7e74f0ed5c444f3bac74e268073679c8233e4ef276d4e9b0aa7ea8240"
          }
