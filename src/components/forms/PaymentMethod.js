...
export default function PaymentMethod(props) {
    const { network, paymentToken: token, setPaymentToken: setToken } = useFun()
    ...
    return (
        <div className="w-full mt-10">
            ...
            <div
                className="button flex justify-between items-center w-full p-4 text-[#344054] font-medium mt-3"
                style={{
                    background: token == nativeToken.symbol ? "#2D4EA214" : "",
                    borderColor: token == nativeToken.symbol ? "#2D4EA2" : "#E4E7EC"
                }}
                onClick={() => {
                    setToken(nativeToken.symbol)
                }}>
                <div className="flex items-center">
