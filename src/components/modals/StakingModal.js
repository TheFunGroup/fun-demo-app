...
import { getEtherBalance, getERC20Balance } from "../../scripts/wallet";
...
export default function StakingModal(props) {
      ...
      wallet
        ...
        .then((addr) => {
          ...
          getEtherBalance(addr)
            .then((balance) => {
              const roundedEther =
                Math.round(parseFloat(formatEther(balance)) * 100000) / 100000;
  ...
  useEffect(() => {
    ...
    if (paymentToken !== "gasless") {
      ...
      handleStakeEth(wallet, normalizedPaymentToken, gasInput, eoa, true)
        .then(async (estimate) => {
          if (estimate.success) {
            console.log(estimate.receipt);
            const usd = await toUSD(
              paymentToken,
              formatEther(estimate.receipt)
            );
            setGas(
              `${parseFloat(formatEther(estimate.receipt)).toFixed(
                6
              )} ${paymentToken} · $${usd}`
            );
          } else {
  ...
  }, [eoa, paymentToken, setPaymasterAddress, setPaymentAddr, stakeInput, wallet]);
      ...
      {mustFund && (
        <div className="alert w-full flex justify-between -mb-[80px] relative">
          ...
          <div
            ...
            onClick={() => router.push(`/fund?example=stake`)}
          >
            Fund
          </div>
        </div>
          ...
          <div className="flex items-start">
            ...
            <div className="flex flex-col justify-start ml-1">
              ...
              <div className="text-xs text-[#667085]">
                {" "}
                At the current rate of stETH, it may be more profitable to swap
                instead of stake. Consider using the swap?
              </div>
            </div>
          </div>
          <div
